...
public:
    ...
    // ── 基础读写 ──
    // Read: 实时数据读取 (位置/血量等频繁变化的字段)
    // 使用 NOCACHE 绕过 VMM 缓存 + NOPAGING_IO 减少总线等待
    ...
    // ReadMeta: 元数据读取 (类名/命名空间/父类指针等不变数据)
    // 允许 VMM 缓存 + 允许分页 I/O — 更可靠，减少 DMA 总线压力
    bool ReadMeta(DWORD64 addr, void* buf, DWORD size) const {
        if (!hVMM || !pid || !addr || !buf || !size) return false;
        DWORD bytesRead = 0;
        const BOOL ok = VMMDLL_MemReadEx(hVMM, pid, addr, (PBYTE)buf, size, &bytesRead, 0);
        return ok && bytesRead == size;
    }
    template<typename T> T ReadMeta(DWORD64 addr) const {
        T v{}; ReadMeta(addr, &v, sizeof(T)); return v;
    }
