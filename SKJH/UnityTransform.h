...
inline bool SKJH_ReadTransformNode(DWORD64 array, int32_t index,
    ...
    const DWORD64 offset = static_cast<DWORD64>(index) *
        g_RuntimeOffsets.unityTransformStride;
    // Transform node data (position/rotation/scale) is dynamic — use NOCACHE
    return mem.Read(array + offset, &node, sizeof(node)) ||
           mem.ReadMeta(array + offset, &node, sizeof(node));
}
...
inline SKJH_UnityTransformResult SKJH_ReadUnityTransform(DWORD64 managedTransform) {
    ...
    // Structural reads (pointers, indices) are immutable — use ReadMeta
    // (VMM-cached) to avoid PCIe bus contention with other threads.
    DWORD64 nativeTransform = mem.ReadMeta<DWORD64>(
    ...
    DWORD64 transformData = mem.ReadMeta<DWORD64>(
    ...
    int32_t index = -1;
    if (!mem.ReadMeta(nativeTransform + g_RuntimeOffsets.unityNativeIndex,
                      &index, sizeof(index))) {
    ...
    DWORD64 transforms = mem.ReadMeta<DWORD64>(
        transformData + g_RuntimeOffsets.unityDataTransforms);
    if (!Mem::IsUserAddress(transforms)) {
        transforms = mem.Read<DWORD64>(
            transformData + g_RuntimeOffsets.unityDataTransforms);
    }
    DWORD64 parents = mem.ReadMeta<DWORD64>(
        transformData + g_RuntimeOffsets.unityDataParents);
    if (!Mem::IsUserAddress(parents)) {
        parents = mem.Read<DWORD64>(
            transformData + g_RuntimeOffsets.unityDataParents);
    }
    ...
    int32_t parent = -2;
    if (!mem.ReadMeta(parents + static_cast<DWORD64>(index) * sizeof(int32_t),
    ...
    while (parent >= 0 && depth++ < 128) {
        ...
        if (!SKJH_IsFiniteVector(result.position) || !SKJH_NormalizeQuaternion(result.rotation))
            return {};
        int32_t next = -2;
        // Parent indices are structural — use ReadMeta
        if (!mem.ReadMeta(parents + static_cast<DWORD64>(parent) * sizeof(int32_t),
                          &next, sizeof(next))) {
