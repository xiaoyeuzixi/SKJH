...
int main(int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        else if (argument == "--bone-probe") boneProbe = true;
        else if (argument == "--signature-check") signatureCheck = true;
    ...
    if (configCheck) {
        ...
        const bool originalPlayerSkeleton = g_ShowPlayerSkeleton;
        const bool originalBotSkeleton = g_ShowBotSkeleton;
        ...
        g_ShowPlayerSkeleton = originalPlayerSkeleton;
        g_ShowBotSkeleton = originalBotSkeleton;
        SKJH_UpdateSkeletonSamplingState();
        const bool restored = SaveGlobalConfig();
        printf("Config check: loaded=%s saved=%s reloaded=%s skeleton=%s rawhid=%s restored=%s mode=%d\n",
            loaded ? "true" : "false",
            saved ? "true" : "false",
            reloaded ? "true" : "false",
            skeletonRoundTrip ? "true" : "false",
            rawHidRoundTrip ? "true" : "false",
            restored ? "true" : "false", g_DisplayMode);
        return saved && reloaded && skeletonRoundTrip && rawHidRoundTrip && restored
            ? 0 : 5;
    }
    ...
    if (boneProbe) {
        ...
        SKJH_ResolveRuntimeTypeInfoSignatures(false, 250);
        // Bone diagnostics deliberately use the compiled SDK first and run
        // even when complete runtime validation is unavailable. A full AOB
        // scan here would hide the failing chain behind a long diagnostic.
        const bool valid = SKJH_RunBoneProbe(probeOutput);
        printf("Bone probe: %s (%s)\n", valid ? "valid" : "invalid",
               probeOutput.string().c_str());
        mem.Close();
        return valid ? 0 : 4;
    }
    ...
    constexpr DWORD kStartupSignatureBudgetMs = 250;
    const bool runtimeTypeInfoReady =
        SKJH_ResolveRuntimeTypeInfoSignatures(false, kStartupSignatureBudgetMs);
    ...
    std::atomic_bool stopRuntimeSdkRetry{false};
    std::thread runtimeSdkRetry;
    if (!runtimeTypeInfoReady) {
        runtimeSdkRetry = std::thread([&stopRuntimeSdkRetry]() {
            constexpr int kMaxAttempts = 3;
            constexpr DWORD kAttemptBudgetMs = 5000;
            for (int attempt = 0;
                 attempt < kMaxAttempts && !stopRuntimeSdkRetry.load();
                 ++attempt) {
                // The final attempt escalates to a full module AOB scan so a
                // game update that moved the TypeInfo slots still resolves the
                // camera/entity/critical managers instead of leaving the overlay
                // permanently blind.
                const bool forceScan = (attempt + 1 == kMaxAttempts);
                if (SKJH_ResolveRuntimeTypeInfoSignatures(
                        forceScan, kAttemptBudgetMs)) return;
                if (attempt + 1 == kMaxAttempts) return;

                // Keep shutdown responsive without adding delay to readers/rendering.
                for (int slice = 0;
                     slice < 10 && !stopRuntimeSdkRetry.load(); ++slice) {
                    Sleep(100);
                }
                if (!stopRuntimeSdkRetry.load()) mem.RefreshTlb();
            }
        });
    }
    ...
    stopRuntimeSdkRetry.store(true);
    GameStop(t0, t1, t2, t3, t4, t5, t6);
    if (runtimeSdkRetry.joinable()) runtimeSdkRetry.join();
    mem.Close();
    return 0;
}
